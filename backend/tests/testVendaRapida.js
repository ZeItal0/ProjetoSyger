...
async function main() {
    ...
    if (!usuario) {
        console.log("nenhum usuario encontrado. Crie um usuario antes.");
        return;
    }
    ...
    if (!variacao) {
        console.log("nenhuma variacao encontrada. Crie um prato e variacao antes.");
        return;
    }
    ...
    console.log("estoque atualizado apos venda.");
}

main()
    .then(() => {
        console.log("\n=== TESTE DE VENDA RAPIDA FINALIZADO ===");
        process.exit(0);
    })
    .catch((err) => {
        console.error(err);
